...
"ow * 9.1 + col * 4.7) > 0.55) {\n          const colors = ['#1a1a28','#"+
"c03040','#f0e8e0','#203060','#e8e8f0','#102040','#ffffff'];\n          "+
"ctx.fillStyle = colors[(row * 17 + col * 13) % colors.length];\n       "+
"   ctx.fillRect(x + 1, y - 3, 3, 4);\n        }\n      }\n    }\n    // ai"+
"sle concrete stripes\n    ctx.fillStyle = 'rgba(180,190,200,0.35)';\n   "+
" for (const ax of [0.18, 0.5, 0.82]) {\n      ctx.fillRect(W * ax - 4, "+
"upperTop, 8, upperH);\n    }\n\n    // --- Upper dark fascia + CLT [crown"+
"] FC ---\n    const fascia1Y = upperTop + upperH;\n    const fasciaH = H"+
" * 0.028;\n    ctx.fillStyle = '#0a0c12';\n    ctx.fillRect(0, fascia1Y,"+
...
"\n      // soft interior lights\n      if ((i + Math.floor(performance.n"+
"ow() / 800)) % 3 === 0) {\n        ctx.fillStyle = 'rgba(255,220,160,0."+
...
"rete band + section numbers ---\n    const concY = fascia2Y + fasciaH *"+
" 0.7;\n    const concH = H * 0.038;\n    ctx.fillStyle = '#c5cbd4';\n    "+
"ctx.fillRect(0, concY, W, concH);\n    ctx.fillStyle = '#1a1a1a';\n    c"+
"tx.font = 'bold ' + Math.floor(W * 0.042) + 'px Arial';\n    ctx.textAl"+
"ign = 'center';\n    ctx.textBaseline = 'middle';\n    ctx.fillText('201"+
"', W * 0.28, concY + concH / 2);\n    ctx.fillText('202', W * 0.72, con"+
...
"'];\n          ctx.fillStyle = colors[(row * 19 + col * 11) % colors.le"+
"ngth];\n          ctx.fillRect(x, y - 4, 3, 5);\n        }\n      }\n    }"+
"\n\n    // pitch-side ad boards sit just above the shared grass/goal lin"+
"e\n    const adH = H * 0.028;\n    const grassTop = H * field.grassTop;\n"+
"    const adY = grassTop - adH;\n    // Fill any gap between lower seat"+
"s and boards (keeps bowl continuous)\n    if (adY > lowerTop + lowerH) "+
"{\n      ctx.fillStyle = '#4a96d0';\n      ctx.fillRect(0, lowerTop + lo"+
"werH, W, adY - (lowerTop + lowerH));\n    } else if (adY < lowerTop + l"+
...
"nst grass = ctx.createLinearGradient(0, grassTop, 0, H);\n    grass.add"+
"ColorStop(0, '#2d8a3e');\n    grass.addColorStop(1, '#1a5a28');\n    ctx"+
".fillStyle = grass;\n    ctx.fillRect(0, grassTop, W, H - grassTop);\n  "+
"  for (let i = 0; i < 8; i++) {\n      ctx.fillStyle = i % 2 ? 'rgba(25"
);
